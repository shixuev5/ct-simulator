...
#define REG_TOTAL_ACTIVE_POWER 0x52  // 合相有功功率 (82), Signed, 0.01W
#define REG_PHASE_A_VOLTAGE 0x40     // A相电压 (64), Unsigned, 0.01V
...
void IRAM_ATTR zeroCrossISR() {
  ...
  if (now - lastZeroCrossTime > ISR_DEBOUNCE_MICROS) {
    ...
    zeroCrossDetected = true; // 设置相位重置标志
  }
}
...
bool readMeterData() {
  ...
  if (result == modbus.ku8MBSuccess) {
    // 根据文档，功率数据是Signed类型，需要处理负值
    uint32_t powerRaw = ((uint32_t)modbus.getResponseBuffer(0) << 16) | modbus.getResponseBuffer(1);
